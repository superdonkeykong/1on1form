...
////基本的に使い回しできる様になっているので、以下の設定を改めて行う必要はありませんが、念のため書いておきます。
////コードの書き換えが発生するのは1on1が10回戦以上行われる場合です。
...
// 2.質問を作成してください。プルダウン方式で、質問タイトルは「round1」から「round9」までの9つです。プルダウン式のround1を作成した後コピーすると楽です。
// *(この設定はデフォルトです。9回戦行われることを想定しています。これより回数が増減する場合は別途マニュアルを参照してください。)
// 3.会社名を入力する欄を作成してください。
// 4.「設定」から「回答」を選択。「メールアドレスを収集する」「回答のコピーを回答者に送信」をオン、「IxxxGxxx と信頼できる組織のユーザーに限定する」をオフにしてください。
// 5.google form編集画面の右上「⋮」からスクリプトエディタをひらいてください。
////
...
// 3.「round1」~「round9」それぞれ、A列にクライアント名、B列に1を記入してください。一行目は必ず見出しをつけてください。
// *別途サンプルシートを作成しているのでそれを参照してください。
